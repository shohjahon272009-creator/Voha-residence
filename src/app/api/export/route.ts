...
export async function GET() {
  ...
  const projects = await db.prepare('SELECT id, name_uz, city, district FROM projects').all() as any[];
   
  const apartments = await db.prepare('SELECT project_id, floor, number, rooms, area, price_cash, status FROM apartments ORDER BY project_id, floor, number').all() as any[];
